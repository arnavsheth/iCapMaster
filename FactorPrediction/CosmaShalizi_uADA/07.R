...
bootstrap.ci <- function(statistic=NULL, simulator=NULL, tboots=NULL,
  ...
  intervals <- bootstrap(tboots, summarizer=equitails, alpha=alpha)
    # If this is a vector or 1D array, make it into a 1*2 array to simplify
    # the next steps
  # if (is.null(dim(intervals)) || dim(intervals)==2) {
  #    intervals <- array(intervals, dim=c(1,2))
  # } else {
  #    # Otherwise, we should transpose it so the upper and lower limits are
  #    # columns
