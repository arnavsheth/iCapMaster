""" models for data generating process """
import numpy as np
...
from services.rainbow_portfolio_analytics.models.minimax_tilting_sampler import TruncatedMVN

class Statistic(Enum):
    """ class for statistic constants """
    MEAN = 'mean'
...
BURNIN_LENGTH = 10000
CHAIN_LENGTH = 10000 + BURNIN_LENGTH
...
MAX_CHAINLENGTH = 100000
NUMERICAL_ZERO = np.sqrt(np.finfo(np.float64).eps)
GAMMA_THRESHOLD = 2.1
PHI_LOWER_BOUND = -np.inf
PHI_UPPER_BOUND = np.inf
...
class DataGeneratingProcess(object):
    ...
    def __init__(self, hyper_parameters: HyperParameter, y: np.ndarray, F: np.ndarray, cash_returns: np.ndarray):
        ...
        self.trunc_gamma_draws: np.ndarray = self.trunc_gamma.rvs(
            size=CHAIN_LENGTH+1,
            alpha=hyper_parameters.alphanu0,
            zeta=hyper_parameters.zetanu0
        )
        self.trunc_gamma_logpdfs: np.ndarray = self.trunc_gamma.logpdf(
            self.trunc_gamma_draws,
            alpha=hyper_parameters.alphanu0,
            zeta=hyper_parameters.zetanu0
        )
        self.nu_draw_idx = 0
        ...
        initial_standard_normals =  DR.standard_normal(size=(CHAIN_LENGTH+1,self.K+self.T))
        self.parameters: Dict[Parameter,np.ndarray] = {Parameter.PSI: np.zeros((CHAIN_LENGTH+1,self.T)),
                                                        ...
                                                        Parameter.PHI : np.zeros((CHAIN_LENGTH+1,self.P)),
                                                        ...
                                                        Parameter.X : initial_standard_normals[:,self.K:]}
        ...
        self.parameters[Parameter.TAU_Y][0] = 1/np.var(self._get_y_tilda(x_Fb) - \
                                                       self._get_Xl_tilda_dot_phi(x_Fb, self.parameters[Parameter.PHI][0,:]), ddof=1)
        ...
        self.trunc_gamma_logpdfs[0] = self.trunc_gamma.logpdf(
            self.parameters[Parameter.NU][0],
            alpha=hyper_parameters.alphanu0,
            zeta=hyper_parameters.zetanu0
        )
        self.parameters[Parameter.PSI][0,:] = get_gamma_mean(
            np.ones((self.T,))*(self.parameters[Parameter.NU][0]*0.5+0.5),
            self.parameters[Parameter.TAU_X][0] * \
                self.parameters[Parameter.TAU_Y][0]*0.5*(x_minus_x_Fb)**2 + \
                    self.parameters[Parameter.NU][0]*0.5
        )
        self.initialization_timer: float = (datetime.now() - start_time).total_seconds()

    # def get_backcasted_returns(self, full_factor_returns: np.ndarray, full_cash_returns: np.ndarray)->np.ndarray:
        
    #     backcasted_xF = full_factor_returns.dot(self.parameters[Parameter.BETA][BURNIN_LENGTH:self.current_iteration,:].T) + \
    #                                                                                             full_cash_returns.reshape(-1,1)
    #     all_phis = [np.concatenate([phi,1 - self.iota_matrix_transpose.dot(phi).reshape(-1)]) 
    #                     for phi in self.parameters[Parameter.PHI][BURNIN_LENGTH:self.current_iteration,:]]
    #     backcasted_yF = np.mean(np.concatenate([np.concatenate([np.roll(cFb,i).reshape(-1,1) 
    #                             for i in range(phi_tilda.shape[0])],axis=1).dot(phi_tilda[::-1])[phi_tilda.shape[0]:].reshape(-1,1) 
    #                                             for cFb, phi_tilda in zip(backcasted_xF.T,all_phis)],axis=1),axis=1)
        
    #     return backcasted_yF
    ...
    def get_aggregate_metrics(self)->Dict[str, Dict[str,Union[float,np.ndarray]]]:
        ...
        yF_values = norm(
            mu_y,
            np.outer(1/np.sqrt(self.parameters[Parameter.TAU_Y][BURNIN_LENGTH:self.current_iteration]),np.ones((1,self.S))),size=mu_y.shape
        )
    ...
    def update_parameters_randomly(self):
        ...
        functions_to_call = [
            self._get_updated_psi,
            self._get_updated_beta,
            self._get_updated_gamma,
            self._get_updated_nu,
            self._get_updated_omega,
            self._get_updated_phi,
            self._get_updated_tau_beta,
            self._get_updated_tau_phi,
            self._get_updated_tau_x,
            self._get_updated_tau_y,
            self._get_updated_x
        ]
    ...
    def _check_if_converged(self)->Tuple[bool,int, np.ndarray]:
        ...
        parameters_to_test: np.ndarray = np.concatenate(
            [param.reshape(1,-1) if param.ndim==1 else param.transpose() for param in parameters_to_test]
        )
        ...
        bootstrap_means: np.ndarray = np.concatenate(
            [get_block_bootstrap_means(param_idx).reshape(1,-1) 
        ...
        if not converged:
            additional_draws = min(
                MAX_CHAINLENGTH-self.current_iteration,
                int(min(10,(MIN_ESS/min(ess_of_parameters))*(1+0.2))*num_draws)
            )
    ...
    def _extend_parameter_arrays(self,N: int):
        ...
        if N==0 or self.current_iteration==1:
            return
        start_time = datetime.now()
        new_standard_normals = DR.standard_normal(size=(N,self.K+self.T))
        for param in Parameter:
            self.parameters[param] = np.append(
                self.parameters[param],np.zeros((N,)) if self.parameters[param].ndim==1 
                    else np.zeros((N,self.parameters[param].shape[1]))
                        if param not in [Parameter.X, Parameter.BETA]
                        else new_standard_normals[:,:self.K]
                            if param==Parameter.BETA
                            else new_standard_normals[:,self.K:],axis=0
            )
    ...
    def _get_updated_phi(self):
        ...
        try:
            self.parameters[param][self.current_iteration,:] = TruncatedMVN(
                mu_phi,
                Sigma_phi,
                self.lower_bound_for_phi,
                self.upper_bound_for_phi
            ).sample(1).reshape(-1)
        except Exception:
            self.parameters[param][self.current_iteration,:] = TruncatedMVN(
                mu_phi,
                fix_spd_matrix(Sigma_phi),
                self.lower_bound_for_phi,
                self.upper_bound_for_phi
            ).sample(1).reshape(-1)
    ...
    def _get_updated_x(self):
        ...
        cholesky, mu_x, _ = lp.dposv(
            Lambda_x_by_tau_y,Phi_mat.transpose().dot(self.y) + tau_x*self._get_conditional_explained_x()*psi,lower=1
        )
        cholesky = np.tril(cholesky)
        self.parameters[param][self.current_iteration, :] = lp.dtrtrs(
            cholesky,
            self.parameters[param][self.current_iteration, :]/sqrt_tau_y,
            lower=1,trans=1
        )[0] + mu_x
        self.active_index[param] += 1
        self.timers[param] += (datetime.now() - start_time).total_seconds()
    ...
    def _get_updated_tau_y(self):
        ...
        zetay = self.hyper_parameters.zetay0 + 0.5*(vec1.dot(vec1) + tau_phi*vec2.dot(m0*vec2) + \
                tau_x*vec3.dot(psi*vec3) + tau_x*tau_beta*(vec4.dot(vec4*(d**2)*a0)))
    ...
    def _get_updated_beta(self):
        ...
        cholesky, mu_beta, _ = lp.dposv(
            Lambda_beta_by_tau_x_tau_y,
            self.F.transpose().dot(psi*(self._get_active_param_value(Parameter.X) - self.cash_returns)) + \
            tau_beta*d*a0*(d*beta0+betadelta0),lower=1
        )
        cholesky = np.tril(cholesky)
        self.parameters[param][self.current_iteration, :] = lp.dtrtrs(
            cholesky,
            self.parameters[param][self.current_iteration, :]/sqrt_tau_x_tau_y,lower=1,
            trans=1
        )[0] + mu_beta
        self.active_index[param] += 1
        self.timers[param] += (datetime.now() - start_time).total_seconds()
    ...
    def _get_stats_sigma2(self)->Dict[str, Dict[str,Union[float,np.ndarray]]]:
        beta_ones: np.ndarray = np.concatenate(
            [
                self.parameters[Parameter.BETA],
                np.ones((self.parameters[Parameter.BETA].shape[0],1))
            ],axis=1
        )[BURNIN_LENGTH:self.current_iteration]
