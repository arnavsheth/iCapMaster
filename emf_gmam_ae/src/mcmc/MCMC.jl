...
function baseline_initmcmc(::Val{:priorexpectation}, ::Type{TΘ};dims,data,hyper, 
  ...
  τx = 1/max(var(x - F*β - r), minvar)*(TΘ <: DGPModelParametersGT ? 1.0 : 1/τy)
...
function baseline_initmcmc(::Val{:nodata},::Type{TΘ};dims,hyper, data,
minvar=eps(Float64)^0.5, additionalΘlist...) where TΘ<:AbstractModelParameters

  @unpack K, T, P, S, s2allts, s2t, Δt = dims
  @assert T==S==0
  @assert isempty(s2allts)
  @assert isempty(s2t)
...
function mcmc(tapedeck::Ttapedeck; dgp::DGP, 
  ...
  while true
    ...
    #@eval Main numrecords=$numrecords
    #@eval Main maxrecords=$maxrecords
