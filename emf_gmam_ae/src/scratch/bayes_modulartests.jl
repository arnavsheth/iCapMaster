...
function formtestdata(; 
  ...
  hyper = m.DGPModelParameters(;
    ...
    β = (;
      ...
      A0 = gencorrelateddata(;rows=nsim, cols=K) |> X->X'X ./ (10*T)),      
    γ = (;),
    ω = (;
      κ0=1.0,
      δ0=1.0),    
  ...
  Θ = m.DGPModelParameters(;x=xtest, ϕ=ϕtest, β=βtest, γ=γtest, ω=ωtest, τy=τytest, τx=τxtest, ψ=ψtest, ν=νtest)
...
function testpostϕ(dgp; iter)
  ...
  @unpack S,P,T,R = dims
  @unpack ϕ0, M0,  ϕmin, ϕmax = hyper
  @unpack x, ϕ, τy, τx, ψ = Θ
  @unpack y = data
  ...
  lqϕ1Mc1 = -τy/2*((ỹ-X̃L*ϕ)'* (ỹ-X̃L*ϕ)+(ϕ-ϕ0)'*M0*(ϕ-ϕ0))
  @assert c1 + lqϕ1Mc1 ≈ post
 
  lqϕ2Mc2 = -τy/2*(ϕ'*(X̃L'*X̃L+M0)*ϕ-(ỹ'*X̃L+ϕ0'*M0)*ϕ - ϕ'*(X̃L'*ỹ+M0*ϕ0))
  c2 = c1-τy/2*(ỹ'*ỹ+ϕ0'*M0*ϕ0)
  @assert c2 + lqϕ2Mc2 ≈ post  

  #now we can test the updating parameters
  Λϕ = τy*(X̃L'*X̃L+M0)
  μϕ = τy*inv(Λϕ)*(X̃L'*ỹ+M0*ϕ0)
  ...
  Random.seed!(11)
  manualdraw = m.draw(:ϕ; μϕ, Σϕ=m.pdinv(Λϕ), Λϕ, ϕmin, ϕmax)
  Random.seed!(11)
  ...
  dgpmom = m.DGP(dgp; x=vbx, τy=vbτy, strict=false)

  @unpack μϕ,Λϕ = m.updateθqϕ(dgpmom, testmoments=true)
  @assert Λϕ ≈ Eτy*(EX̃LtX̃L+M0)
  @assert μϕ ≈ Eτy*inv(Λϕ)*(EX̃Ltỹ+M0*ϕ0)
...
function testpostτy(dgp; iter)
  ...
  @unpack data, Θ, dims,hyper = dgp
  @unpack S,P,T,K = dims
  @unpack αy0, ζy0, M0, ϕ0, β0, βΔ0, A0, v, ϕmin, ϕmax = hyper
  @unpack x, ϕ, β, τy, τx, ψ, γ, ν = Θ
  @unpack y,F,r = data
  ...
  @assert lpdist(dgp,:ϕprior) ≈ (-τy/2*(ϕ-ϕ0)'*M0*(ϕ-ϕ0) + 0.5*(P*log(τy)-P*log(2π)+log(det(M0))) ) 
  @assert lpdist(dgp,:βprior) ≈ (-(τy*τx/2)*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0)
    + 0.5*(K*log(τy*τx)-K*log(2π)+log(det(D*A0*D))))
  @assert lpdist(dgp,:xprior) ≈ (-(τy*τx/2)*(x-F*β-r)'*Ψ*(x-F*β-r) + 0.5*(T*log(τy*τx) - T*log(2π)+log(det(Ψ))))
  @assert lpdist(dgp,:ylike) ≈ (-(τy/2)*(ỹ-X̃L*ϕ)'*(ỹ-X̃L*ϕ) + 0.5*(S*log(τy) - S*log(2π)))
  ...
  lqτyMc1 = (-τy/2*(ỹ-X̃L*ϕ)'*(ỹ-X̃L*ϕ)
    ...
    -τy*τx/2*(β̃-Dinv*βΔ0)'*D*A0*D*(β̃-Dinv*βΔ0)
    -ζy0*τy+((S+T+K+P)/2+αy0-1)*log(τy))
  c1=(-(S+T+P+K)/2*log(2π) + 0.5*log(det(Ψ)) + 0.5*log(det(M0))
    +0.5*log(det(D*A0*D)) + (T+K)/2*log(τx) + αy0*log(ζy0) - loggamma(αy0)
    +sum([:γprior,:ωprior,:νprior,:ψprior,:τxprior] .|> p->lpdist(dgp, p)))  
  ...
  ζy = (0.5*(ỹ'ỹ + ϕ'*X̃L'*X̃L*ϕ - ỹ'*X̃L*ϕ - ϕ'*X̃L'*ỹ)
    +0.5*(ϕ'*M0*ϕ + ϕ0'*M0*ϕ0 - ϕ0'*M0*ϕ - ϕ'*M0*ϕ0)
    +0.5*τx*(x'*Ψ*x + (β'*F'+r')*Ψ*(F*β+r) - x'*Ψ*(F*β+r) - (β'*F'+r')*Ψ*x)
    +0.5*τx*((β-β0)'*D*A0*D*(β-β0) + βΔ0'*A0*βΔ0 - βΔ0'*A0*D*(β-β0) - (β-β0)'*D*A0*βΔ0) + ζy0)
  ...
  @assert ζy ≈ (0.5*(Eỹtỹ + EϕtX̃LtX̃Lϕ - EX̃Ltỹ'*μϕ - μϕ'*EX̃Ltỹ)
    + 0.5*(EϕtM0ϕ + ϕ0'M0*ϕ0 - ϕ0'M0*μϕ - μϕ'*M0*ϕ0)
    + 0.5*Eτx*(ExtΨx + EβtFtPrtΨFβPr - μx'*EΨ*(F*μβ+r) - (μβ'F'+r')*EΨ*μx)
    + 0.5*Eτx*(EβtDA0Dβ - 2*β0'*EDA0D*μβ + β0'EDA0D*β0
...
function testpostτx(dgp; iter)
  ...
  c1 = (-(T+K)/2*log(2π)+0.5*log(det(Ψ)) +0.5*log(det(D*A0*D))
    +(T+K)/2*log(τy)+αx0*log(ζx0)-loggamma(αx0)
    +sum([:ylike, :ϕprior, :γprior,:ωprior,:ψprior,:τyprior,:νprior] .|> p->lpdist(dgp, p))) 
  @assert lqτxMc1+c1 ≈ post "lqτxMc1+c1: $(lqτxMc1+c1); post: $post"

  #conditional updating equations
  αx=(T+K)/2+αx0
  ζx = (ζx0 + 0.5*τy*(x'*Ψ*x + (β'*F'+r')*Ψ*(F*β +r)- x'*Ψ*(F*β +r)- (β'*F'+r')*Ψ*x)
    +0.5*τy*(β̃'*D*A0*D*β̃ + βΔ0'*A0*βΔ0 - βΔ0'*A0*D*β̃ - β̃'*D*A0*βΔ0))
...
function testpostβ(dgp; iter)
  ...
  lqβMc1 = (-τy*τx/2*(x-F*β-r)'*Ψ*(x-F*β-r) - τy*τx/2*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0))
  c1 = ((T+K)/2*log(τx*τy/(2π))+0.5*log(det(Ψ)) +0.5*log(det(D*A0*D))
    +sum([:ylike, :ϕprior, :γprior,:ωprior,:ψprior,:νprior,:τyprior,:τxprior] .|> p->lpdist(dgp, p))) 
  @assert lqβMc1+c1 ≈ post "lqβMc1+c1: $(lqβMc1+c1); post: $post"

  lqβMc2 = -τy*τx/2*(β'F'Ψ*F*β - (x-r)'Ψ*F*β - β'F'Ψ*(x-r)+ β'D*A0*D*β - 2*β'D*A0*(D*β0+βΔ0))
  c2 = c1 - τy*τx/2*((D*β0+βΔ0)'*A0*(D*β0+βΔ0)+(x-r)'*Ψ*(x-r))
  @assert lqβMc2 + c2 ≈ post "lqβMc2+c2: $(lqβMc2 + c2); post: $post" 

  #conditional updating equations
  Λβ=τx*τy*(F'*Ψ*F + D*A0*D)
  μβ=τx*τy*inv(Λβ)*(F'*Ψ*(x-r) + D*A0*(D*β0+βΔ0))
  ...
  dgpmom = m.DGP(dgp; γ=vbγ, x=vbx,
    ψ=vbψ, τx=vbτx, τy=vbτy,strict=false)

  #compute and verify q(β)
  @unpack μβ,Λβ = m.updateθqβ(dgpmom)
  @assert Λβ ≈ Eτx*Eτy*(F'*EΨ*F+EDA0D)


  @assert μβ ≈ (Eτx*Eτy*inv(Λβ)*(F'*EΨ*(μx-r)+EDA0D*β0+ED*A0*βΔ0)) "
    μβ=$μβ but Eτx*Eτy*inv(Λβ)*(F'*EΨ*(μx-r)+EDA0D*β0+ED*A0*βΔ0)=$(Eτx*Eτy*inv(Λβ)*(F'*EΨ*(μx-r)+EDA0D*β0+ED*A0*βΔ0))!"
...
function testpostγ(dgp, A0::Diagonal; iter)
  ...
  @unpack data, Θ, dims,hyper = dgp
  @unpack T,K = dims
  @unpack β0, βΔ0, v, κ0, δ0 = hyper
  @unpack x, β, τy, τx, γ, ω = Θ
  @unpack F = data
  ...
  for k ∈ 1:K
    Mk = Not(k)
    lqγkMc1 = log(d[k])-τx*τy*d[k]^2*a0[k]/2*(β[k]-β0[k]-βΔ0[k]/d[k])^2+γ[k]*log(ω)+(1-γ[k])*log(1-ω)
    c1 = (0.5*log(τx*τy*a0[k]/(2π)) + logpdf(MvNormal(βΔ0[Mk]./d[Mk].+β0[Mk],Diagonal(τx*τy.*d[Mk].^2 .* a0[Mk]) |> inv), β[Mk])
      + sum(γ[Mk] .|> γj-> γj*log(ω)+(1.0-γj)*log(1-ω))
      + sum([:ylike, :xprior, :ϕprior, :ψprior,:νprior, :ωprior,:τyprior,:τxprior] .|> p->lpdist(dgp, p))) 
    @assert lqγkMc1+c1 ≈ post "lqγkMc1+c1: $(lqγkMc1+c1); post: $post"

    lqγkMc2 = log(d[k]) - τx*τy*a0[k]/2*((β[k]-β0[k])^2*d[k]^2 - 2*(β[k]-β0[k])βΔ0[k]d[k])+γ[k]*log(ω) + (1-γ[k])*log(1-ω)
    c2=c1 - τx*τy*a0[k]/2*βΔ0[k]^2
    @assert lqγkMc2+c2 ≈ post "lqγkMc2+c2: $(lqγkMc2+c2); post: $post"


    lp1=- τx*τy*a0[k]/2*((β[k]-β0[k])^2 - 2*(β[k]-β0[k])*βΔ0[k]) + log(ω)
    lp0=-log(v) - τx*τy*a0[k]/2*((β[k]-β0[k])^2/v^2 - 2*(β[k]-β0[k])*βΔ0[k]/v) + log(1-ω)
    lh = lp0 < lp1 ? lp1 : lp0
  ...
  dgpmom = m.DGP(dgp; β=vbβ, τx=vbτx, τy=vbτy, ω =vbω, strict=false)
  ...
  lp̃γ1check=(-Eτx*Eτy .* a0) ./ 2 .* (Eβ̃2 .- 2(μβ-β0).*βΔ0) .+ Elogω
  lp̃γ0check=(-Eτx*Eτy .* a0) ./ 2 .* (Eβ̃2 ./ v^2 .- 2(μβ-β0).*βΔ0 ./ v) .+ Elog1Mω .- log(v)
  ...
  p̃γ1check=exp.((-Eτx*Eτy .* a0) ./ 2 .* (Eβ̃2 .- 2(μβ-β0).*βΔ0) .+ Elogω)
  p̃γ0check=exp.((-Eτx*Eτy .* a0) ./ 2 .* (Eβ̃2 ./ v^2 .- 2(μβ-β0).*βΔ0 ./ v) .+ Elog1Mω .- log(v))
...
function testpostγ(dgp, A0::Matrix; iter)
  ...
  for k ∈ 1:K
    Mk = Not(k)
    lqγkMc1 = log(d[k])-τx*τy/2*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0)+γ[k]*log(ω)+(1-γ[k])*log(1-ω)
    c1 = (K/2*log(τx*τy/2π) + sum(log.(d[Mk])) + 0.5*logdet(A0) + sum(γ[Mk] .|> γj-> γj*log(ω)+(1.0-γj)*log(1-ω))
      + sum([:ylike, :xprior, :ϕprior, :ψprior,:νprior, :ωprior,:τyprior,:τxprior] .|> p->lpdist(dgp, p))) 
    @assert lqγkMc1+c1 ≈ post "lqγkMc1+c1: $(lqγkMc1+c1); post: $post"


    lqγkMc2 = log(d[k]) - τx*τy/2*((β-β0)'D*A0*D*(β-β0)-2(β-β0)'*D*A0*βΔ0)+γ[k]*log(ω) + (1-γ[k])*log(1-ω)
    c2=c1 - τx*τy/2*βΔ0'A0*βΔ0
    @assert lqγkMc2+c2 ≈ post "lqγkMc2+c2: $(lqγkMc2+c2); post: $post"

    DGdk1 = deepcopy(D)
    DGdk1[k,k] = 1.0
    lp1= -τx*τy/2*((β-β0)'DGdk1*A0*DGdk1*(β-β0)-2(β-β0)'*DGdk1*A0*βΔ0) + log(ω)
    DGdkv = DGdk1 |> deepcopy
    DGdkv[k,k] = 1/v
    lp0= -log(v) - τx*τy/2*((β-β0)'DGdkv*A0*DGdkv*(β-β0)-2(β-β0)'*DGdkv*A0*βΔ0) + log(1-ω)
  ...
  for k ∈ 1:K
    ...
    lp̃γ1k = -(Eτx*Eτy/2)*(Eβ̃tDA0Dβ̃Gdk1k-2*Eβ̃tDA0βΔ0Gdk1k)+Elogω
    lp̃γ0k = -(Eτx*Eτy/2)*(Eβ̃tDA0Dβ̃Gdkvk-2*Eβ̃tDA0βΔ0Gdkvk)+Elog1Mω-log(v)
...
function testpostω(dgp; iter)
  ...
  c1=-logbeta(κ0,δ0) + sum([:ylike, :ϕprior, :xprior, :βprior, :ψprior,:νprior,:τyprior,:τxprior] .|> p->lpdist(dgp, p)) 
...
function testpostψ(dgp; iter, checkνnorm=true)
  ...
  for t ∈ 1:T
    ...
    c1 = (0.5*log(τx*τy/(2π)) + ν/2*log(ν/2)-loggamma(ν/2)
      ...
      +sum([:ylike, :ϕprior, :γprior,:ωprior,:τyprior,:τxprior, :βprior, :νprior] .|> p->lpdist(dgp, p)))
...
function testpostν(dgp; iter)
  ...
  c1 = αν0*log(ζν0)-loggamma(αν0) - log(denom) + sum([:ylike, :ϕprior, :γprior,:ωprior,:τyprior,:τxprior,:βprior,:xprior] .|> p->lpdist(dgp, p))
...
@time testupdates(; S=80, Δt=3, P=6, K=20, iter=10^5, )
