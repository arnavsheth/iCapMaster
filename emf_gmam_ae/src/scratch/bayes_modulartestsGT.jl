...
function formtestdata(; 
  ...
  Θ = m.DGPModelParametersGT(;x=xtest, ϕ=ϕtest, β=βtest, γ=γtest, ω=ωtest, τy=τytest, τx=τxtest, 
...
#the log pdf
function lpdist( dgp::m.DGP{<:m.DGPModelParametersGT}, s::Symbol)
  ...
  (s ≡ :ylike) && return logpdf(MultivariateNormal(m.formΦ(ϕ; dims)*x, I(dims.S)/τy),data.y)
  (s ≡ :xprior) && return logpdf(MultivariateNormal(data.F * β+data.r, inv(m.formΨ(ψ)) ./ (τx) ),x)
  (s ≡ :ϕprior ) && return logpdf(MultivariateNormal(ϕ0, (inv(M0) |> Symmetric) .* (1/ τy / τϕ )),ϕ)
  ...
  if s ≡ :βprior #βprior is a bit more complex than the others
    ...
    return logpdf(MultivariateNormal(β0 + Dinv*βΔ0, Symmetric(inv(D*A0*D)) .* (1 / (τβ*τx)) ),β)
  end
...
function testpostϕ(dgp::m.DGP{<:m.DGPModelParametersGT}; iter)
...
function testpostx(dgp; iter)
  ...
  X̃L = m.formX̃L(;x, dims)
  Φ = m.formΦ(ϕ; dims)
  @assert all(X̃L*ϕ .+ x[dims.s2t] .≈ Φ*x)

  Ψ = ψ |> m.formΨ
  post = lpdist(dgp, :post)

  #verify the prior and likelihood
  @assert lpdist(dgp,:xprior) ≈ (-(τx/2)*(x-F*β-r)'*Ψ*(x-F*β-r) +0.5*(T*log(τx)-T*log(2π)+log(det(Ψ))))
  ...
  lqx1Mc1 = -τy/2*(y-Φ*x)'*(y-Φ*x) - τx/2*(x-F*β-r)'*Ψ*(x-F*β-r)
  c1 = ((S)/2*log(τy/(2π))+T/2*log(τx/(2π))+0.5*log(det(Ψ))
  ...
  lqx2Mc2=-x'*(τy/2*Φ'*Φ + τx/2 .* Ψ)*x + (τy/2*y'*Φ + τx/2 * (β'*F'+r')*Ψ)*x + x'*(τy/2*Φ'*y + τx/2 .* Ψ*(F*β+r))
  c2 = c1  - τy/2*y'*y - τx/2*(β'*F'+r')*Ψ*(F*β+r)
  @assert lqx2Mc2 + c2 ≈ post "lqx2Mc2=$(lqx2Mc2 + c2); post=$post"

  Λx=(τy.*Φ'*Φ+τx.*Ψ)
  μx=inv(Λx)*(τy.*Φ'*y+τx*Ψ*(F*β+r))
  ...
  @assert Λx ≈ (Eτy*EΦtΦ+Eτx*EΨ) "sum(Λx)=$(Λx) while sum(Eτy*(EΦtΦ+Eτx*EΨ)) =\n $(sum(Eτy*(EΦtΦ+Eτx*EΨ)))!
   \n EΦtΦ=$EΦtΦ, \n EΨ=$EΨ"
  @assert μx ≈ inv(Λx)*(Eτy*EΦ'y+Eτx*EΨ*(F*μβ+r))
...
function testpostτy(dgp; iter)
  ...
  @unpack data, Θ, dims,hyper = dgp
  @unpack S,P,T,K = dims
  @unpack αy0, ζy0, M0, ϕ0, v, αϕ0, ζϕ0, ϕmin, ϕmax= hyper
  @unpack x, ϕ, τy, ψ, τx, γ, ν, τϕ = Θ
  @unpack y,F,r = data
  ...
  lqτyMc1 = (-τy/2*(ỹ-X̃L*ϕ)'*(ỹ-X̃L*ϕ)
    ...
    -ζy0*τy+((S+P)/2+αy0-1)*log(τy))
  c1=(-(S+P)/2*log(2π) + 0.5*log(det(M0)) + 0.5*P*log(τϕ) 
     + αy0*log(ζy0) - loggamma(αy0)
    +sum([:γprior,:ωprior,:νprior,:ψprior, :xprior,:τxprior, :τβprior, :τϕprior, :βprior] .|> p->lpdist(dgp, p)))  
  ...
  ζy = (0.5*(ỹ'ỹ + ϕ'*X̃L'*X̃L*ϕ - ỹ'*X̃L*ϕ - ϕ'*X̃L'*ỹ)
    +0.5*τϕ*(ϕ'*M0*ϕ + ϕ0'*M0*ϕ0 - ϕ0'*M0*ϕ - ϕ'*M0*ϕ0) + ζy0)
  ...
  dgpmom = m.DGP(dgp; 
    x=vbx, 
    ϕ=vbϕ, 
    τϕ=vbτϕ,strict=false)


  @unpack αy,ζy = m.updateθqτy(dgpmom)
  @assert αy ≈ (S+P)/2+αy0
  @assert ζy ≈ (0.5*(Eỹtỹ + EϕtX̃LtX̃Lϕ - EX̃Ltỹ'*μϕ - μϕ'*EX̃Ltỹ)
    + 0.5*Eτϕ*(EϕtM0ϕ + ϕ0'M0*ϕ0 - ϕ0'M0*μϕ - μϕ'*M0*ϕ0) + ζy0)
  lqτyθ = (αy-1.0)*log(τy) .- ζy*τy 


  lqτyθsim = 0.0
  sim = Dict( :ϕtX̃LtX̃Lϕ=>0.0,  :SX̃LtX̃L=>0.0,
    :ϕtM0ϕ=>0.0,  :ỹtỹ=>0.0, :τϕ=>0.0)

  d = (;μϕ, Λϕ, αx,ζx,μx, Λx, EXL)
  @eval Main d=$d
  ...
  for i ∈ 1:iter
    ...
    simỹ = m.formỹ(data.y, Θi.x; dims)
...
function testpostτx(dgp; iter)
  ...
  @assert lpdist(dgp,:βprior) ≈ (-(τx/2*τβ)*(β̃-Dinv*βΔ0)'*D*A0*D*(β̃-Dinv*βΔ0)
    + 0.5*(K*log(τx)-K*log(2π)+log(det(D*A0*D))+K*log(τβ)))
  @assert lpdist(dgp,:xprior) ≈ (-(τx/2)*(x-F*β-r)'*Ψ*(x-F*β-r) + 0.5*(T*log(τx) - T*log(2π)+log(det(Ψ))))
  @assert lpdist(dgp,:τβprior) ≈ (αβ0*log(ζβ0)-loggamma(αβ0) + (αβ0-1)*log(τβ)-ζβ0*τβ)  


  lqτxMc1 = (-τx/2*(x-F*β-r)'*Ψ*(x-F*β-r)
    -τx/2*τβ*(β̃-Dinv*βΔ0)'*D*A0*D*(β̃-Dinv*βΔ0)
    -τx*ζx0+((T+K)/2+αx0-1)*log(τx))
  c1 = (-(T+K)/2*log(2π)+0.5*log(det(Ψ)) +0.5*log(det(D*A0*D))
    +αx0*log(ζx0)-loggamma(αx0)+0.5*K*log(τβ)
  ...
  #conditional updating equations
  αx=(T+K)/2+αx0
  ζx = (ζx0 + 0.5*(x'*Ψ*x + (β'*F'+r')*Ψ*(F*β +r)- x'*Ψ*(F*β +r)- (β'*F'+r')*Ψ*x)
    +0.5*τβ*(β̃'*D*A0*D*β̃ + βΔ0'*A0*βΔ0 - βΔ0'*A0*D*β̃ - β̃'*D*A0*βΔ0))
  ...
  dgpmom = m.DGP(dgp; x=vbx, γ=vbγ, 
    β=vbβ, ψ=vbψ,τβ=vbτβ, strict=false)

  @unpack αx,ζx = m.updateθqτx(dgpmom)
  @assert αx ≈(T+K)/2+αx0
  @assert ζx ≈ (ζx0 + 0.5*(ExtΨx+EβtFtPrtΨFβPr-μx'EΨ*(F*μβ+r)-(μβ'*F'+r')*EΨ*μx) 
    +0.5*Eτβ*(Eβ̃tDA0Dβ̃  + βΔ0'A0*βΔ0 - βΔ0'A0*ED*(μβ-β0) -  (μβ-β0)'ED*A0*βΔ0))
...
function testpostτβ(dgp; iter)
  ...
  lqτβMc1 = (-τx/2*τβ*(β̃-Dinv*βΔ0)'*D*A0*D*(β̃-Dinv*βΔ0)
    -ζβ0*τβ+(αβ0-1+K/2)*log(τβ))
  c1 = (K/2*log((τx/ 2π))+0.5*log(det(D*A0*D))
  ...
  #conditional updating equations
  αβ=K/2+αβ0
  ζβ = ζβ0 + 0.5*τx*(β̃'*D*A0*D*β̃ + βΔ0'*A0*βΔ0 - βΔ0'*A0*D*β̃ - β̃'*D*A0*βΔ0)
  ...
  @assert αβ ≈ K/2+αβ0
  @assert ζβ ≈ (ζβ0 + 0.5*Eτx*(Eβ̃tDA0Dβ̃ + βΔ0'A0*βΔ0 - βΔ0'A0*ED*(μβ-β0) -  (μβ-β0)'ED*A0*βΔ0))
...
function testpostβ(dgp; iter)
  ...
  #verify the prior and likelihood
  @assert lpdist(dgp,:βprior) ≈ (-(τx/2*τβ)*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0)
    + 0.5*(K*log(τx*τβ)-K*log(2π)+log(det(D*A0*D))))
  @assert lpdist(dgp,:xprior) ≈ (-(τx/2)*(x-F*β-r)'*Ψ*(x-F*β-r) + 0.5*(T*log(τx) - T*log(2π)+log(det(Ψ))))

  lqβMc1 = (-τx/2*(x-F*β-r)'*Ψ*(x-F*β-r) - τx/2*τβ*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0))
  c1 = ((T+K)/2*log(τx/(2π))+0.5*log(det(Ψ)) +0.5*log(det(D*A0*D)) + 0.5*K*log(τβ)
  ...
  lqβMc2 = -τx/2*(β'F'Ψ*F*β - (x-r)'Ψ*F*β - β'F'Ψ*(x-r)+ (τβ.* (β'D*A0*D*β - 2*β'D*A0*(D*β0+βΔ0))))
  c2 = c1 - τx/2*(τβ*(D*β0+βΔ0)'*A0*(D*β0+βΔ0)+(x-r)'*Ψ*(x-r))
  @assert lqβMc2 + c2 ≈ post "lqβMc2+c2: $(lqβMc2 + c2); post: $post" 

  #conditional updating equations
  Λβ=τx*(F'*Ψ*F + (D*A0*D.*τβ))
  μβ=τx*inv(Λβ)*(F'*Ψ*(x-r) + ( τβ .*D*A0*(D*β0+βΔ0)))
  ...
  dgpmom = m.DGP(dgp; γ=vbγ, x=vbx,τβ=vbτβ,
    ψ=vbψ, τx=vbτx, strict=false)

  #compute and verify q(β)
  @unpack μβ,Λβ = m.updateθqβ(dgpmom)
  @assert Λβ ≈ Eτx*(F'*EΨ*F+(EDA0D .*Eτβ))
  @assert μβ ≈ Eτx*inv(Λβ)*(F'*EΨ*(μx-r)+(Eτβ.*( EDA0D*β0+ED*A0*βΔ0)))
...
function testpostγ(dgp, A0::Diagonal; iter)
  ...
  for k ∈ 1:K
    Mk = Not(k)
    lqγkMc1 = log(d[k])-τx*d[k]^2*τβ*a0[k]/2*(β[k]-β0[k]-βΔ0[k]/d[k])^2+γ[k]*log(ω)+(1-γ[k])*log(1-ω)
    c1 = (0.5*log(τx*τβ *a0[k]/(2π)) + logpdf(MvNormal(βΔ0[Mk]./d[Mk].+β0[Mk],Diagonal(τx*τβ .*d[Mk].^2 .* a0[Mk]) |> inv), β[Mk])
    ...
    lqγkMc2 = log(d[k]) - τx*τβ*a0[k]/2*((β[k]-β0[k])^2*d[k]^2 - 2*(β[k]-β0[k])*βΔ0[k]d[k])+γ[k]*log(ω) + (1-γ[k])*log(1-ω)
    c2=c1 - τx*τβ*a0[k]/2*βΔ0[k]^2
    @assert lqγkMc2+c2 ≈ post "lqγkMc2+c2: $(lqγkMc2+c2); post: $post"


    lp1=- τx*τβ*a0[k]/2*((β[k]-β0[k])^2 - 2*(β[k]-β0[k])*βΔ0[k]) + log(ω)
    lp0=-log(v) - τx*τβ*a0[k]/2*((β[k]-β0[k])^2/v^2 - 2*(β[k]-β0[k])*βΔ0[k]/v) + log(1-ω)
    lh = lp0 < lp1 ? lp1 : lp0
  ...
  dgpmom = m.DGP(dgp; β=vbβ, τx=vbτx, τβ=vbτβ, ω =vbω, strict=false)
  ...
  lp̃γ1check=(-Eτx*Eτβ .* a0) ./ 2 .* (Eβ̃2 .- 2(μβ-β0).*βΔ0) .+ Elogω
  lp̃γ0check=(-Eτx*Eτβ .* a0) ./ 2 .* (Eβ̃2 ./ v^2 .- 2(μβ-β0).*βΔ0 ./ v) .+ Elog1Mω .- log(v)
  ...
  p̃γ1check=exp.((-Eτx*Eτβ .* a0) ./ 2 .* (Eβ̃2 .- 2(μβ-β0).*βΔ0) .+ Elogω)
  p̃γ0check=exp.((-Eτx*Eτβ .* a0) ./ 2 .* (Eβ̃2 ./ v^2 .- 2(μβ-β0).*βΔ0 ./ v) .+ Elog1Mω .- log(v))
...
function testpostγ(dgp, A0::Matrix; iter)
  ...
  #verify the prior and likelihood
  @assert lpdist(dgp,:βprior) ≈ (-(τx*τβ/2)*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0)
    + 0.5*(K*log(τx)-K*log(2π)+log(det(D*A0*D))+K*log(τβ)))
  @assert lpdist(dgp,:γprior) ≈ ((γ .|> γk-> log(ω^(γk)*(1-ω)^(1-γk))) |> sum)
  ...
  for k ∈ 1:K
    Mk = Not(k)
    lqγkMc1 = log(d[k])-τx*τβ/2*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0)+γ[k]*log(ω)+(1-γ[k])*log(1-ω)
    c1 = (K/2*log(τx*τβ/2π) + sum(log.(d[Mk])) + 0.5*logdet(A0) + sum(γ[Mk] .|> γj-> γj*log(ω)+(1.0-γj)*log(1-ω))
      + sum([:ylike, :xprior, :ϕprior, :ψprior,:νprior, :ωprior,:τyprior,:τxprior, :τϕprior, :τβprior] .|> p->lpdist(dgp, p))) 



    lqγkMc2 = log(d[k]) - τx*τβ/2*((β-β0)'D*A0*D*(β-β0)-2(β-β0)'*D*A0*βΔ0)+γ[k]*log(ω) + (1-γ[k])*log(1-ω)
    c2=c1 - τx*τβ/2*βΔ0'A0*βΔ0
    @assert lqγkMc2+c2 ≈ post "lqγkMc2+c2: $(lqγkMc2+c2); post: $post"

    DGdk1 = deepcopy(D)
    DGdk1[k,k] = 1.0
    lp1= -τx*τβ/2*((β-β0)'DGdk1*A0*DGdk1*(β-β0)-2(β-β0)'*DGdk1*A0*βΔ0) + log(ω)
    DGdkv = DGdk1 |> deepcopy
    DGdkv[k,k] = 1/v
    lp0= -log(v) - τx*τβ/2*((β-β0)'DGdkv*A0*DGdkv*(β-β0)-2(β-β0)'*DGdkv*A0*βΔ0) + log(1-ω)
  ...
  for k ∈ 1:K
    ...
    lp̃γ1k = -(Eτx*Eτβ/2)*(Eβ̃tDA0Dβ̃Gdk1k-2*Eβ̃tDA0βΔ0Gdk1k)+Elogω
    lp̃γ0k = -(Eτx*Eτβ/2)*(Eβ̃tDA0Dβ̃Gdkvk-2*Eβ̃tDA0βΔ0Gdkvk)+Elog1Mω-log(v)
...
function testpostψ(dgp; iter, checkνnorm=true)
  ...
  for t ∈ 1:T
    ft = F[t,:]
    lqψtMc1 = (-τx*ψ[t]/2*(x[t]- ft'*β-r[t])^2 + (ν/2-1/2)*log(ψ[t])-ν * ψ[t]/2)
    c1 = (0.5*log(τx/(2π)) + ν/2*log(ν/2)-loggamma(ν/2)
      +sum(logpdf(Normal(F[j,:]'*β+r[j], 1/(τx*ψ[j])^0.5),x[j]) for j ∈ setdiff(1:T,t))
    ...
    @assert αψ[t] ≈ ν/2+1/2
    @assert ζψ[t] ≈  τx/2*(x[t]^2 - 2x[t]*(ft'*β+r[t]) + (β'*ft+r[t])^2)+ν/2
  ...
  dgpmom = m.DGP(dgp; x=vbx, τx=vbτx, 
  ...
  for t ∈ 1:T
    f = F[t,:]
    @assert ζψ[t] ≈ (Eτx/2*(Ex2[t] - 2μx[t]*(F[t,:]'*μβ+r[t]) + EβtfPrftβPr[t])) .+ Eν/2
  end
...
@time testupdates(; S=50, Δt=1, P=2, K=6, iter=10^7, )
