...
using Random, DataFrames, StatsBase, BenchmarkTools, UnPack, ProgressMeter, Dates, StatsPlots
...
function formtestdata(; 
  ...
  dates = Date(2005,1,1):Month(1):(Date(2005,1,1)+Month(S*Δt+P-1)) |> collect .|> lastdayofmonth
  dims=m.Dims(;S,P,K,Δt,dates,testdims=true,
    Fβs = ["β_$k" for k ∈ 1:K])
  ...
  Θ = m.DGPModelParametersG(;x=xtest, ϕ=ϕtest, β=βtest, γ=γtest, ω=ωtest, τy=τytest, τx=τxtest, 
...
function testpriordist(dgp::m.DGP{<:m.DGPModelParametersG})

  @assert typeof(dgp.data) <: m.AbstractData{<:m.HasData}



  1:dgp.records.numchains .|> chainnum-> m.record!(dgp; chainnum, Θ=dgp.Θ)
  @assert !isempty(dgp.records)

  dgp0 = m.prioronlydgp(dgp)
  @eval Main dgp0=$dgp0
  @assert typeof(dgp0) <: m.DGP{<:m.AbstractModelParametersG, <:m.AbstractData{<:m.NoData}} "
  typeof(dgp0): $(typeof(dgp0))"

  @unpack hyper, Θ, dims,records = dgp0
  @unpack ϕ0,M0,v,β0,βΔ0,A0 = hyper
  @unpack ϕ,τy,τϕ,γ,β,ω,ν = Θ
  @unpack K,P,S,T = dims 
  @assert S==T==0
  @assert isempty(records)
  post = lpdist(dgp0, :post)
  alldists = [:ylike, :ϕprior, :xprior, :βprior, :γprior,:ωprior,:ψprior,
    :τxprior, :νprior, :τyprior,:τβprior, :τϕprior] 
  @assert sum(alldists.|> p->lpdist(dgp0, p)) ≈ post

  #start with ϕ
  lpϕMc = - τy*τϕ/2*(ϕ-ϕ0)'*M0*(ϕ-ϕ0)
  cϕ = P/2*log(τy*τϕ/(2π)) + 0.5*logdet(M0)
  @unpack μϕ,Λϕ = m.updateθpϕ(dgp0)
  @assert μϕ == ϕ0
  @assert Λϕ ≈ (M0*τy*τϕ|> Symmetric)
  liketestϕ = (lpϕMc +cϕ + sum(setdiff(alldists, [:ϕprior]).|> p->lpdist(dgp0, p)))
  @assert liketestϕ ≈ post "
  liketestϕ !≈ post: liketestϕ=$liketestϕ, post=$post"

  throw("looks good to ϕ")
  #τy
  m.formD(γ,v)
  lpτyMc = -τy*τϕ/2*(ϕ-ϕ0)'*M*(ϕ-ϕ0) - (-τy*τx*τβ/2)*(β-β0-Dinv*βΔ0)*D'A0*D*(β-β0-Dinv*βΔ0)
  cβ = (P+K)/2*log(τy/2π) + (K/2)*log(τx)
...
function testpostx(dgp; iter)
  ...
  #verify the prior and likelihood
  @assert lpdist(dgp,:xprior) ≈ (-(τy*τx/2)*(x-F*β-r)'*Ψ*(x-F*β-r) +0.5*(T*log(τy*τx)-T*log(2π)+log(det(Ψ))))
  ...
  lqx1Mc1 = -τy/2*((y-Φ*x)'*(y-Φ*x) + τx*(x-F*β-r)'*Ψ*(x-F*β-r))
  c1 = ((S+T)/2*log(τy/(2π))+T/2*log(τx)+0.5*log(det(Ψ))
  ...
  lqx2Mc2=-τy/2*(x'*(Φ'*Φ + τx .* Ψ)*x - (y'*Φ + τx * (β'*F'+r')*Ψ)*x-x'*(Φ'*y + τx.*Ψ*(F*β+r)))
  c2 = c1  - τy/2*y'*y - τx*τy/2*(β'*F'+r')*Ψ*(F*β+r)
  @assert lqx2Mc2 + c2 ≈ post

  Λx=τy.*(Φ'*Φ+τx.*Ψ)
  μx=τy.*inv(Λx)*(Φ'*y+τx*Ψ*(F*β+r))
  ...
  @assert Λx ≈ Eτy*(EΦtΦ+Eτx*EΨ) "sum(Λx)=$(Λx) while sum(Eτy*(EΦtΦ+Eτx*EΨ)) =\n $(sum(Eτy*(EΦtΦ+Eτx*EΨ)))!
   \n EΦtΦ=$EΦtΦ, \n EΨ=$EΨ"
  @assert μx ≈ Eτy*inv(Λx)*(EΦ'y+Eτx*EΨ*(F*μβ+r))
...
function testpostτy(dgp; iter)
  ...
  c1=(-(S+T+P+K)/2*log(2π) + 0.5*log(det(Ψ)) + 0.5*log(det(M0)) + 0.5*P*log(τϕ) + 0.5*K*log(τβ)
    +0.5*log(det(D*A0*D)) + (T+K)/2*log(τx) + αy0*log(ζy0) - loggamma(αy0)
  ...
  ζy = (0.5*(ỹ'ỹ + ϕ'*X̃L'*X̃L*ϕ - ỹ'*X̃L*ϕ - ϕ'*X̃L'*ỹ)
    ...
    +0.5*τx*(x'*Ψ*x + (β'*F'+r')*Ψ*(F*β+r) - x'*Ψ*(F*β+r) - (β'*F'+r')*Ψ*x)
  ...
  vbx = m.VBqx(dgp; μx, Λx, testmoments=true)
  @unpack EXL, EXLtXL, EX̃LtX̃L, EX̃L, Eỹ, EX̃Ltỹ, Eỹtỹ, EX̃Ltỹ, Σx = vbx
  vbτx = m.VBqτx(dgp; αx, ζx, testmoments=true)
  @unpack Eτx= vbτx
  vbτϕ = m.VBqτϕ(dgp; αϕ, ζϕ, testmoments=true)
  @unpack Eτϕ= vbτϕ
  ...
  EβtFtPrtΨFβPr = m.moment(:EβtFtPrtΨFβPr; μβ, Σβ, F, EΨ, r)
  ...
  m.testmoment(:EβtFtPrtΨFβPr; μβ, Λβ, F, EΨ, r)
  ...
  @assert ζy ≈ (0.5*(Eỹtỹ + EϕtX̃LtX̃Lϕ - EX̃Ltỹ'*μϕ - μϕ'*EX̃Ltỹ)
    ...
    + 0.5*Eτx*(ExtΨx + EβtFtPrtΨFβPr - μx'*EΨ*(F*μβ+r) - (μβ'F'+r')*EΨ*μx)
  ...
  lqτyθsim = 0.0
  sim = Dict(:xtΨx=>0.0, :βtFtPrtΨFβPr=>0.0, :ϕtX̃LtX̃Lϕ=>0.0,  :SX̃LtX̃L=>0.0,
    :ϕtM0ϕ=>0.0, :DA0D=>zeros(K,K), :β̃tDA0Dβ̃=>0.0, :ỹtỹ=>0.0,:X̃Ltỹ=>zeros(P),
    :τβ=>0.0, :τϕ=>0.0)
  ...
  for i ∈ 1:iter
    ...
    sim[:βtFtPrtΨFβPr] += (Θi.β'*F'+r')*simΨ*(F*Θi.β+r)
  ...
  println("βtFtPrtΨFβPr: sim $(sim[:βtFtPrtΨFβPr]); act $(EβtFtPrtΨFβPr) ")
...
function testpostτx(dgp; iter)
  ...
  lqτxMc1 = (-τy*τx/2*(x-F*β-r)'*Ψ*(x-F*β-r)
    -τy*τx/2*τβ*(β̃-Dinv*βΔ0)'*D*A0*D*(β̃-Dinv*βΔ0)
    -τx*ζx0+((T+K)/2+αx0-1)*log(τx))
  c1 = (-(T+K)/2*log(2π)+0.5*log(det(Ψ)) +0.5*log(det(D*A0*D))
  ...
  #conditional updating equations
  αx=(T+K)/2+αx0
  ζx = (ζx0 + 0.5*τy*(x'*Ψ*x + (β'*F'+r')*Ψ*(F*β +r)- x'*Ψ*(F*β +r)- (β'*F'+r')*Ψ*x)
  ...
  @assert αx ≈(T+K)/2+αx0
  @assert ζx ≈ (ζx0 + 0.5*Eτy*(ExtΨx+EβtFtPrtΨFβPr-μx'EΨ*(F*μβ+r)-(μβ'*F'+r')*EΨ*μx) 
    +0.5*Eτy*Eτβ*(Eβ̃tDA0Dβ̃  + βΔ0'A0*βΔ0 - βΔ0'A0*ED*(μβ-β0) -  (μβ-β0)'ED*A0*βΔ0))
  lqτxθ = (αx - 1.0)*log(τx) .- ζx*τx 


  lqτxθsim = 0.0
  sim = Dict(:xtΨx=>0.0, :βtFtPrtΨFβPr=>0.0, 
...
function testpostβ(dgp; iter)
  ...
  lqβMc1 = (-τy*τx/2*(x-F*β-r)'*Ψ*(x-F*β-r) - τy*τx/2*τβ*(β-β0-Dinv*βΔ0)'*D*A0*D*(β-β0-Dinv*βΔ0))
  c1 = ((T+K)/2*log(τx*τy/(2π))+0.5*log(det(Ψ)) +0.5*log(det(D*A0*D)) + 0.5*K*log(τβ)
  ...
  lqβMc2 = -τy*τx/2*(β'F'Ψ*F*β - (x-r)'Ψ*F*β - β'F'Ψ*(x-r)+ (τβ.* (β'D*A0*D*β - 2*β'D*A0*(D*β0+βΔ0))))
  ...
  μβ=τx*τy*inv(Λβ)*(F'*Ψ*(x-r) + ( τβ .*D*A0*(D*β0+βΔ0)))
  ...
  @assert Λβ ≈ Eτx*Eτy*(F'*EΨ*F+(EDA0D .*Eτβ))
  @assert μβ ≈ Eτx*Eτy*inv(Λβ)*(F'*EΨ*(μx-r)+(Eτβ.*( EDA0D*β0+ED*A0*βΔ0)))
...
function testpostψ(dgp; iter, checkνnorm=true)
  ...
  for t ∈ 1:T
    ft = F[t,:]
    lqψtMc1 = (-τy*τx*ψ[t]/2*(x[t]- ft'*β-r[t])^2 + (ν/2-1/2)*log(ψ[t])-ν * ψ[t]/2)
    c1 = (0.5*log(τx*τy/(2π)) + ν/2*log(ν/2)-loggamma(ν/2)
      +sum(logpdf(Normal(F[j,:]'*β+r[j], 1/(τx*τy*ψ[j])^0.5),x[j]) for j ∈ setdiff(1:T,t))
    ...
    @assert αψ[t] ≈ ν/2+1/2
    @assert ζψ[t] ≈  τx*τy/2*(x[t]^2 - 2x[t]*(ft'*β+r[t]) + (β'*ft+r[t])^2)+ν/2
  ...
  @unpack Eτx= vbτx
  vbβ = m.VBqβ(dgp; μβ, Λβ, testmoments=true)
  @unpack EβtfPrftβPr= vbβ 
  vbν = m.VBqν(dgp; lpν, pν, η1, testmoments=true)
  @unpack Eν= vbν
  

  #compute and test the necessary moments
  m.testmoment(:EβtfPrftβPr; F, μβ, Λβ,r)
  ...
  for t ∈ 1:T
    f = F[t,:]
    @assert ζψ[t] ≈ (Eτx*Eτy/2*(Ex2[t] - 2μx[t]*(F[t,:]'*μβ+r[t]) + EβtfPrftβPr[t])) .+ Eν/2
  end
  ...
  lqψθsim = zeros(T)
  sim = Dict(:x2=>zeros(T), :βtfPrftβPr=>zeros(T), :αψ=>zeros(T), :ζψ=>zeros(T), :ν=>0.0, :νnorm=>0.0)
  ...
  for i ∈ 1:iter
    ...
    Θi = dgpi.Θ
    sim[:x2] .+= Θi.x .* Θi.x
    sim[:βtfPrftβPr] .+=  ((f,rt)->(Θi.β'*f .+ rt)*(f'*Θi.β .+ rt)).(eachrow(F),r)
    sim[:ν] += Θi.ν
...
function testposty(dgp; iter)
  ...
  @assert sum(Φ*(F*β+r)) ≈ m.derived(Val{:EyF}(), dgp; testderived=true)[:sumEyF]
  ...
  for i ∈ 1:iter
    ...
    ψ = m.draw(:ψ; αψ=dgp.Θ.ν/2, ζψ=2/dgp.Θ.ν, T)
    #x = 
  end
...
function testextendfactordata(dgp;
  modelfactors=m.PARAM[:livemodelfactors],
  Frf = m.PARAM[:factorFrf],
  factorfrequency=m.PARAM[:factorfrequency],)

  m.extendfactordata(dgp;  modelfactors, Frf, factorfrequency,)
end

function testmeanmodel(dgp; iter)
  @info "testing mean model (iter=$iter)"
  #forget the moments for now- just check the conditional posterior. We will come back to the moments.
  dgpmean = m.MeanModel(dgp)
  @unpack data, dims, Θ, hyper = dgpmean
  @unpack y=data
  @unpack S=dims
  @unpack μ0,τμ0, α0, ζ0 = hyper
  
  #we will pick different values for testing purposes
  @assert dgpmean.Θ.μ ≈ mean(y)
  @assert dgpmean.Θ.τ  ≈ 1/var(data.y)
  μ = 0.2
  τ = 0.25
  dgpmean = m.DGP(dgpmean; μ, τ)


  post = lpdist(dgpmean,:post)

  @assert lpdist(dgpmean,:μprior) ≈ -τμ0*τ/2*(μ-μ0)^2+0.5*log(τμ0*τ/(2π))
  @assert lpdist(dgpmean,:τprior) ≈ α0*log(ζ0)-loggamma(α0)+(α0-1)*log(τ)-ζ0*τ
  @assert lpdist(dgpmean, :ylike) ≈ -(y.-μ)'*Diagonal(fill(τ,S))*(y.-μ)/2+S/2*log(τ/(2*π))

  lq0μMc1 = -(y.-μ)'*Diagonal(fill(τ,S))*(y.-μ)/2-τ*τμ0/2*(μ-μ0)^2
  μc1 = (S+1)/2*log(τ/(2π))+0.5*log(τμ0)+lpdist(dgpmean,:τprior)
  @assert lq0μMc1+μc1 ≈ post "lq0μMc1+μc1: $(lq0μMc1+ μc1); post: $post"

  lq0μMc2 = -τ*(S+τμ0)/2*(μ^2-2μ*(sum(y)+τμ0*μ0)*1/(S+τμ0))
  μc2 = -τ*sum(y.^2)/2-τμ0*τ*μ0^2/2+μc1
  @assert lq0μMc2+μc2 ≈ post "lq0μMc2+μc2: $(lq0μMc2+ μc2); post: $post"

  τμ=τ*(S+τμ0)
  μμ = τ/τμ*(sum(y)+τμ0*μ0)

  @assert m.updateθpμ(dgpmean; ).μμ ≈ μμ
  @assert m.updateθpμ(dgpmean; ).τμ ≈ τμ

  lq0μMc3 = -τμ/2*(μ-μμ)^2
  μc3 = μc2 + τμ/2*μμ^2
  @assert lq0μMc3+μc3 ≈ post "lq0μMc3+μc3: $(lq0μMc3+ μc3); post: $post"

  lq0μMc4 = logpdf(Normal(μμ,(τμ)^(-0.5)),μ)
  μc4=μc3 - 0.5*log(τμ/(2π))
  @assert lq0μMc4+μc4 ≈ post "lq0μMc4+μc4: $(lq0μMc4+ μc4); post: $post"

  #mu is marginalized out of tau so the tests need to adjust for the new density function
  lq0τMc1 = ((S+1)/2+(α0-1))*log(τ)-ζ0*τ-τ/2*sum(y .* y) - τ*τμ0/2*μ0^2 + τμ/2*μμ^2-0.5*log(τμ)
  τc1= 0.5*log(τμ0)+α0*log(ζ0)-loggamma(α0)-S/2*log(2π)
  margadj = lq0μMc4
  @assert lq0τMc1+τc1 ≈ post-margadj "lq0τMc1+τc1: $(lq0τMc1+τc1); post-margadj: $(post-margadj)"

  α = S/2+α0
  ζ = 0.5*sum(y .* y)+τμ0/2*μ0^2-(sum(y)+τμ0*μ0)^2/(2*(S+τμ0)) + ζ0
  @assert m.updateθpτ(dgpmean; ).α ≈ α
  @assert m.updateθpτ(dgpmean; ).ζ ≈ ζ


  lq0τMc2=(α-1)*log(τ)-ζ*τ 
  τc2 = τc1 - 0.5*log(S+τμ0)
  @assert lq0τMc2+τc2 ≈ post-margadj "lq0τMc2+τc2: $(lq0τMc2+τc2); post-margadj: $(post-margadj)"

  lq0τMc3 = logpdf(Gamma(α,1/ζ),τ)
  τc3 = -α*log(ζ) + loggamma(α) + τc2
  @assert lq0τMc3+τc3 ≈ post-margadj "lq0τMc3+τc3: $(lq0τMc3+τc3); post-margadj: $(post-margadj)"

  Θchk = (;μ,τ)
  sim = Dict( :μ=>missings(Float64, iter), :μμ=>missings(Float64, iter), :τμ=>missings(Float64, iter), 
    :τ=>missings(Float64, iter), :α=>missings(Float64, iter),:ζ=>missings(Float64, iter),
    :μ_chk=>missings(Float64, iter), :μμ_chk=>missings(Float64, iter), :τμ_chk=>missings(Float64, iter), 
    :τ_chk=>missings(Float64, iter), :α_chk=>missings(Float64, iter),:ζ_chk=>missings(Float64, iter),)

  dgpi = dgpmean |> deepcopy
  for i ∈ 1:iter

    #start with the coded version
    θτi = m.updateθpτ(dgpi  )
    τ = m.draw(:τ; α=θτi.α, ζ=θτi.ζ)
    dgpi = m.DGP(dgpi; τ)
    ...
    θμi = m.updateθpμ(dgpi ; )
    μ = m.draw(:μ; μμ=θμi.μμ, τμ=θμi.τμ)
    dgpi = m.DGP(dgpi; μ)
    ...
    #we check the above against a more traditional mcmc with no marginalization
    αchk = (S+1)/2 + α0
    ζchk = sum(0.5 .* (y .- Θchk.μ).^2) + τμ0/2*(Θchk.μ-μ0)^2 + ζ0
    local τchk = m.draw(:τ; α=αchk, ζ=ζchk)
    ...
    τμchk = τchk*(S+τμ0)
    μμchk = (sum(y)+τμ0*μ0)/τμchk * τchk
    local μchk = m.draw(:μ; μμ=μμchk, τμ=τμchk)
  ...
  results = Dict(k=>"mean=$(mean(sim[k])) std=$(std(sim[k])) " * 
    "skewness=$(skewness(sim[k] .|> Float64)) kurt=$(kurtosis(sim[k] .|> Float64))" for k ∈ keys(sim))

  println("μ: $(results[:μ])")
  println("μchk: $(results[:μ_chk])\n")
  println("μμ: $(results[:μμ])")
  println("μμchk: $(results[:μμ_chk])\n")
  println("τμ: $(results[:τμ])")
  println("τμchk: $(results[:τμ_chk])\n")

  println("τ: $(results[:τ])")
  println("τchk: $(results[:τμ_chk])\n")
  println("α: $(results[:α])")
  println("αchk: $(results[:α_chk])\n")
  println("ζ: $(results[:ζ])")
  println("ζchk: $(results[:ζ_chk])\n")

  for k ∈ keys(results)
    (length(string(k)) ≥ 4) && (string(k)[(end-3):end] ≡ "_chk") && continue
    k ∈ [:α, :ζ] && continue #this would be a meaningless plot
    k_chk = Symbol(string(k),:_chk)
    @assert haskey(results, k_chk) && haskey(sim, k_chk)

    #=plot(qqplot(sim[k][(iter ÷ 2):end],sim[k_chk][(iter ÷ 2):end], qqline=:R)
      ) |> p->savefig(p,"$(m.PARAM[:testpath])/testplots/$(k)_$(iter)_$(SEED)G.png")=#
  end

  #@assert (α-1)*log(τ)-ζ*τ ≈ lq0τMc1 "(α-1)*log(τ)-ζ*τ: $((α-1)*log(τ)-ζ*τ) lq0τMc1: $lq0τMc1"
...
function testupdates(;iter, kwargs...)
  ...
  @eval Main dgp=$dgp
  #m.profilemcmc(:shuffle1, dgp; )

   #=for i ∈ 1:20
    global SEED = i
    Random.seed!(i)
    testmeanmodel(dgp; iter)
   end=#
   #m.testcv(dgp;iter)
   #testpriordist(dgp)
  
  #testextendfactordata(dgp)
...
@time testupdates(; S=30, Δt=3, P=3, K=5, iter=10^5, )
